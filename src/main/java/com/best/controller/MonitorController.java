...
public class MonitorController {
	...
	public String deleteMonitor(HttpServletRequest req, HttpServletResponse resp, Model model) throws IOException, ParseException {
		...
		alertMonitorService.deleteMonitor(Long.parseLong(monitorId.trim()), obj.getUserCount());
	...
	public String monitorDingdanView(HttpServletRequest req, HttpServletResponse resp, Model model) throws IOException,
		...
		if (currentIndexId == 1) {
			// 分仓SKU订单量
			excelFileName = user.getUserCount() + "_" + currentSku + "_IDO_" + startTime + "_" + endTime;
			filePath = statisticService.getWareHouseSkuIdoCount(currentSku, wareHouseCodes, monitor.getMonitorCustomerCode(),
					startTime, endTime, excelFileName);

			model.addAttribute("currentFilePath", "/json/" + currentSku + "/" + filePath);

			model.addAttribute("currentExcelFile", "/json/monitor/dingdang/" + excelFileName + ".xls");

		} else if (currentIndexId == 2) {

		} else if (currentIndexId == 3) {
			// 分仓地区分布
			excelFileName = user.getUserCount() + "_" + currentSku + "_DISTRIBUTED_" + startTime + "_" + endTime;
			statisticService.getDistributedSkuIdoCount(currentSku, monitor.getMonitorCustomerCode(), startTime, endTime,
					excelFileName, model);
		} else {
			excelFileName = user.getUserCount() + "_" + currentSku + "_PERCENTED_" + startTime + "_" + endTime;
			statisticService.getPercentSkuIdoCount(currentSku, monitor.getMonitorCustomerCode(), startTime, endTime,
					excelFileName, model);
		}
