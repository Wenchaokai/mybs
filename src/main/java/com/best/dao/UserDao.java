...
public class UserDao extends BaseDao {
	...
	@SuppressWarnings("unchecked")
	public List<User> findUserByPageSize(String searchName, String searchCount, String customerSelected,
			String warehouseSelected, Integer startIndex) {
		Map<String, Object> map = new HashMap<String, Object>();
		int count = 0;
		...
		if (customerSelected != null && customerSelected.trim().length() > 0 && !customerSelected.startsWith("-1"))
			map.put("customerSelected", "%" + customerSelected + "%");
		if (warehouseSelected != null && warehouseSelected.trim().length() > 0 && !warehouseSelected.startsWith("-1"))
			map.put("warehouseSelected", "%" + warehouseSelected + "%");
		return (List<User>) this.list(space + selectString, map, sqlMapClient);
	}

	public Integer findUserTotalSize(String searchName, String searchCount, String customerSelected, String warehouseSelected) {
		Map<String, Object> map = new HashMap<String, Object>();
		int count = 0;
	...
	public void resetPasswordMember(String userId) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("userId", userId);
		map.put("pwd", CommonUtils.encoder("Abc123"));
		this.update(space + "RESET_USER_PASSWORD", map, sqlMapClient);
	}
